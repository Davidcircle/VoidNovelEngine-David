...
local window_display_selector = require("application.scene.window.window_display_selector")
...
local function _on_tick_menu_bar()
    if imgui.BeginMenuBar() then
        ...
        if imgui.BeginMenu(" 视图 ") then
            ...
            imgui.EndDisabled()
            imgui.Separator()
            if _menu_item_with_icon("slideshow-2-line", "显示器选择...", "F12") then
                window_display_selector.toggle()
            end
            imgui.EndMenu()
        end
...
local function on_enter(self)
    ...
    window_display_selector.on_enter()
end
...
local function on_update(self, delta)
    ...
    imgui.Begin("DockerWindow", nil, flag_window_main_docker)
        ...
        window_display_selector:on_update(delta)
        -- 处理Ctrl+Shift+S全部保存
...
local function on_render(self)
    ...
    -- 渲染独立预览窗口
    if window_display_selector.is_preview_in_window() then
        window_display_selector.render_preview()
    end
end
